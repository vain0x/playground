module Otojson.Json
  ...
  module Primities =
    let private isGenericTypeDef ty (value: Type) =
      value.IsGenericType && value.GetGenericTypeDefinition() = ty

    let private representBy<'T> (toString: obj -> string) (_: Representer) (_: Type) (value: obj) =
      JsonValue.String (toString value)

    let representSeq<'T> (represent: Representer) (_: Type) (value: obj) =
      JsonValue.Array [
        for value in value :?> IEnumerable do
          match value with
          | null ->
            yield JsonValue.Null
          | value ->
            yield represent (value.GetType()) value
      ]

    let representPairSeq<'K, 'V> (represent: Representer) (_: Type) (value: obj) =
      JsonValue.Object [
        for kv in value :?> IEnumerable do
      ]

    let private simpleDef<'T> = {
      Accept = (=) typeof<'T>
      Represent = representBy (box >> string)
    }
    ...
    let internal defaultDefinitions = [
      intDef
      int64Def
      floatDef
      stringDef
